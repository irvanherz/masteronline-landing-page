...
export default function Navbar({ activeKey, theme = 'default' }: NavbarProps) {
    ...
    return (
        <header className={theme === 'red' ? "theme-red fixed top-0 px-4 bg-black bg-opacity-0 inset-x-0 z-40" : "theme-default sticky top-0 bg-white bg-opacity-0 filter backdrop-blur-lg inset-x-0 z-40"} id="navbar">
            <nav className={classNames("md:container mx-auto flex items-center justify-between py-4 filter", theme === 'red' ? 'px-4 md:px-8' : 'px-8')} aria-label="Global" id="navbar-inner">
                        ...
                        <Link
                            ...
                            className={
                                classNames(
                                    ...
                                    activeKey === item.id
                                        ? (theme === 'red' ? 'font-bold text-paprika-100' : 'text-paprika-900')
                                        : (theme === 'red' ? 'text-white' : 'text-black')
                                )
                            }>
                            {item.name}
                        </Link>
                ...
                <div className="hidden lg:flex lg:justify-end gap-4">
                    <a className="px-6 py-2 bg-white border-2 border-paprika-900 text-lg font-semibold font-rubik rounded text-paprika-900 special-btn-effect-1" href="/login">Login</a>
                    <a className="px-6 py-2 bg-paprika-900 border-2 border-paprika-900 text-lg font-semibold font-rubik rounded text-white special-btn-effect-2" href="/register">Daftar</a>
                </div>
            </nav>
                            ...
                            <div className="space-y-2 py-6">
                                {navigation.map((item) => (
                                    <Link
                                        key={item.name}
                                        href={item.href}
                                        className={classNames("-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-white hover:bg-white hover:bg-opacity-10", activeKey === item.id ? 'text-paprika-100' : '')}
                                    >
                                        {item.name}
                                    </Link>
                                ))}
