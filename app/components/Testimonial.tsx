...
const TESTIMONIALS = [
    {
        id: 1,
        user: "Toko Accandi",
        userDesc: <span><span className='font-bold'>Soemardi W</span> - Pemilik</span>,
    ...
    return (
        <div className="py-8 text-white md:text-black">
            <div className="relative md:flex p-8 md:p-0 rounded overflow-hidden">
                <img className='absolute inset-0 w-full h-full object-cover block md:hidden z-0' src={testimonial.photo} />
                ...
                <div className="w-full md:w-8/12 xl:w-9/12 flex flex-col justify-between gap-8 md:gap-16 z-10">
                    ...
                    <div className='inline-flex gap-4 z-10 items-center self-center md:self-start'>
                        ...
                        <div className='flex items-center text-lg md:text-2xl font-bold'>{currentSlide + 1}/{numSlides}</div>
                        <button className='btn btn-primary btn-circle btn-xs md:btn-md' onClick={handleNextSlide}><ChevronRight /></button>
                    </div>
                    <div className="flex items-center gap-4 z-10">
                        <img src={testimonial.logo} className="w-8 h-8 object-cover block md:hidden" alt="" />
                        <div className="flex-1">
                            <div className="text-base md:text-2xl lg:text-3xl xl:text-4xl font-bold">{testimonial.user}</div>
                            <div className="text-xs md:text-lg lg:text-lg xl:text-xl">{testimonial.userDesc}</div>
                        </div>
                    </div>
                </div>
                <div className="flex-none w-4/12 xl:w-3/12 pl-8 hidden md:block">
                    <div className='aspect-w-2 aspect-h-3 relative w-full h-auto rounded overflow-hidden'>
                        ...
                        <img src={testimonial.logo} className="absolute w-32 h-32 left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 object-cover" alt="" />
                    </div>
                </div>
            </div>
        </div>
...
function UsersCarousel() {
    return (
        ...
        >
            {USERS.map(c => (
                <SwiperSlide key={c.id} className='self-center px-4 first:pl-8 last:pr-8'>
    ...
    return (
        <div className="bg-paprika-50 text-black py-12 md:py-18 lg:py-24 space-y-12 md:space-y-18 lg:space-y-24">
            <div className="space-y-4 lg:space-y-8">
                <div className="px-8 text-center text-sm md:text-lg lg:text-xl">Dipercaya ribuan toko terbaik</div>
                <UsersCarousel />
            </div>
