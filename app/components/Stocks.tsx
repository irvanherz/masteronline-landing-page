...
function Stock({ id, lastChangeId, title, className, stock }: StockProps) {
    ...
    useGSAP(() => {
        if (id === lastChangeId) {
            let tween = gsap.to(changeRef.current, {
                opacity: 1,
                duration: 1.2,
                repeat: 1,
                yoyo: true,
                repeatDelay: 0,
                ease: CustomEase.create("custom", "M0,0 C0,0 -0.014,0.872 0.069,0.917 0.156,1.04 1,1.012 1,1.012 "),
            });
            tween.play();
            setDisplayStock(stock)
        } else {
            let tween = gsap.to(stockRef.current, {
                delay: 0.5,
                opacity: 0,
                duration: 0.25,
                repeat: 1,
    ...
    }, { dependencies: [stock, id, lastChangeId, stockRef.current, changeRef.current], scope: containerRef, revertOnUpdate: true })
        ...
        <div className={VARIANTS[id as keyof typeof VARIANTS].containerClassName} ref={containerRef}>
            ...
            <div className='flex items-center gap-2'>
                <div className='flex-none'>
                    ...
                    <div className={classNames(VARIANTS[id as keyof typeof VARIANTS].circleClassName, "block md:hidden")} />
                </div>
                ...
                <div className='flex-none'>
                    <span>Stok:</span>&nbsp;<span className='font-bold' style={{ opacity: 1 }} ref={stockRef}>{displayStock}</span>
                </div>
            </div>
            <div className={VARIANTS[id as keyof typeof VARIANTS].changeNotifClassName} style={{ opacity: 0 }} ref={changeRef}>
                <div className='font-bold text-center text-xl hidden md:block'>Terjual!</div>
                <div className='flex items-center gap-2'>
                    <div className='flex-none hidden md:block'>
                        <img src={`${IMAGE_BASEURL}/images/maroon-beanie.png`} alt='Maroon Beanie' className='aspect-square w-8 rounded' />
                    </div>
                    <div className='flex-1'>
...
export default function Stocks() {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setStockChange(c => {
                const marketplaces = ['shopee', 'lazada', 'blibli', 'bukalapak', 'tiktok', 'tokopedia']
        ...
        <div className='flex flex-col gap-2 max-w-[900px]'>
            <div className="flex flex-col items-start p-4 relative bg-slate-900 rounded overflow-hidden md:hidden">
                <div className="flex items-center gap-2 relative self-stretch w-full flex-auto">
                    <img className="relative w-8 h-8 object-cover" alt="Rectangle" src={`${IMAGE_BASEURL}/images/maroon-beanie.png`} />
                    <div className="relative flex-1 font-semibold text-white text-base">Maroon Beanie</div>
                </div>
            </div>
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:gap-3 bg-white md:bg-transparent bg-opacity-85 rounded overflow-hidden'>
                <Stock stock={stockChange.qty} lastChangeId={stockChange.id} id='tokopedia' title='Tokopedia' />
                <Stock stock={stockChange.qty} lastChangeId={stockChange.id} id='shopee' title='Shopee' />
                <Stock stock={stockChange.qty} lastChangeId={stockChange.id} id='lazada' title='Lazada' />
                <Stock stock={stockChange.qty} lastChangeId={stockChange.id} id='blibli' title='Blibli' />
                <Stock stock={stockChange.qty} lastChangeId={stockChange.id} id='bukalapak' title='Bukalapak' />
                <Stock stock={stockChange.qty} lastChangeId={stockChange.id} id='tiktok' title='TikTok' />
            </div>
        </div>
    )
}
