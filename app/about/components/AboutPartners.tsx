...
function PartnerGroupCard({ title, children, wrapperClassName }: any) {
    return (
        <div className="space-y-8 lg:space-y-12">
            <div className="text-xl md:text-3xl lg:text-5xl font-bold font-rubik">{title}</div>
            ...
            <div className="flex-1 space-y-4">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex-none">
                        <div className="border rounded px-2 py-1 text-xs md:text-sm lg:text-base">{tag}</div>
                    </div>
                </div>
                <div>{desc}</div>
            </div>
...
function PartnerItemCardB({ title, desc, icon, url, className, linkClassName }: any) {
    return (
        <div className={classNames("flex p-4 lg:p-8 rounded items-center gap-4 lg:gap-8 col-span-1 text-sm md:text-lg lg:text-xl", className)}>
            <div className="flex-none">
                <img src={icon} alt={title} className="max-w-20 w-full" />
            </div>
            <div className="flex-1">
                <div className='font-bold text-base md:text-xl lg:text-3.5xl pb-2'>{title}</div>
                <div className='pb-6'>{desc}</div>
                <div><a href={url} target="_blank" className={classNames("font-bold text-paprika-900 hover:text-paprika-900", linkClassName)}>Kunjungi Situs {title} &rarr;</a></div>
            </div>
        </div>
    )
}
        ...
        <div className="flex flex-col gap-4 lg:gap-8 text-sm md:text-lg lg:text-xl lg:px-8 first:pl-0 last:pr-0">
            <div className="flex-none flex items-center gap-4 lg:gap-8">
                ...
                <div className="flex-1 space-y-2">
                    <div className="font-bold text-lg md:text-xl lg:text-2.5xl">{title}</div>
                    <div><a href={url} target="_blank" className="font-bold text-paprika-900 hover:text-paprika-900">Kunjungi Situs &rarr;</a></div>
                </div>
            </div>
        ...
        <div className="flex items-center gap-4 md:gap-8 text-sm md:text-lg lg:text-xl">
            <div className="flex-1 flex flex-col gap-3 md:gap-6">
                <div className='flex gap-4 items-center'>
                    ...
                    <div className="text-lg md:text-xl lg:text-2.5xl font-bold">{title}</div>
                </div>
                <div>{desc}</div>
                <div>
                    <a href={url} target="_blank" className="font-bold text-paprika-900 hover:text-paprika-900">Kunjungi Situs {title} &rarr;</a>
                </div>
            </div>
...
export function AboutPartnerMarketplace() {
    ...
    return (
        <PartnerGroupCard title="Partner Marketplace" wrapperClassName="lg:divide-y -m-4 lg:-m-8">
            {batchedPartners.map((batch, i) => (
                <div key={i} className='grid grid-cols-1 lg:grid-cols-2 lg:divide-x'>
                    {batch.map(c => (
                        <PartnerItemCardA key={c.id} title={c.title} desc={c.desc} icon={c.icon} url={c.url} tag={c.tag} />
                    ))}
                </div>
            ))}
        </PartnerGroupCard>
...
export function AboutPartnerEcommerce() {
    return (
        <PartnerGroupCard title="Partner E-Commerce" wrapperClassName="space-y-4 lg:space-y-8">
            {ECOMMERCE_PARTNERS.map(c => (
                <PartnerItemCardB key={c.id} title={c.title} desc={c.desc} icon={c.icon} url={c.url} className={c.className} linkClassName={c.linkClassName} />
            ))}
        </PartnerGroupCard>
    )
}
...
export function AboutPartnerAccounting() {
    return (
        <PartnerGroupCard title="Partner Accounting/ERP" wrapperClassName="grid grid-cols-1 lg:grid-cols-2 overflow-hidden space-y-4 lg:space-y-0 lg:divide-x">
            {ACCOUNTING_PARTNERS.map(c => (
                <PartnerItemCardC key={c.id} title={c.title} desc={c.desc} icon={c.icon} url={c.url} />
            ))}
        </PartnerGroupCard>
    )
}
...
export function AboutPartnerPOS() {
    return (
        <PartnerGroupCard title="Partner Point Of Sales (POS)">
            {POS_PARTNERS.map(c => (
                <PartnerItemCardD key={c.id} title={c.title} desc={c.desc} icon={c.icon} url={c.url} />
            ))}
        </PartnerGroupCard>
    )
}
